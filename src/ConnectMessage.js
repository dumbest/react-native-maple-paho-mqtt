...
export default class {
  ...
  encode(): ArrayBuffer {
    ...
    if (options.user) {
      remLength += lengthOfUTF8(options.user) + 2;
      if (options.pass) {
        remLength += lengthOfUTF8(options.pass) + 2;
      }
    }
    ...
    if (options.user) {
      pos = writeString(options.user, lengthOfUTF8(options.user), byteStream, pos);
      if (options.pass) {
        writeString(options.pass, lengthOfUTF8(options.pass), byteStream, pos);
      }
    }
    return buffer;
  }
